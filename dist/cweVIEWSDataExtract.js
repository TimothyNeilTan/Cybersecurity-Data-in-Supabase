...
class cweFunctions {
    constructor() { }
    ;
    cweDataExtract(jsonData) {
        const { v4: uuidv4 } = require('uuid');
        const cweData = [];
        let submissionName;
        let submissionDate;
        let submissionOrganization;
        for (const items of jsonData.Weakness_Catalog.Views[0].View) {
            ...
            if (items.Audience) {
                for (const stakeholder of items.Audience[0].Stakeholder) {
                    cweStakeholders.push({ stakeholder: stakeholder.Type,
                        stakeholder_description: stakeholder.Description });
                }
                ;
            }
            else {
                cweStakeholders.push({ stakeholder: "No stakeholders available",
                    stakeholder_description: "No stakeholders available" });
            }
            ;
            if (items.Members) {
                for (const member of items.Members[0].Has_Member) {
                    cweMembers.push({ member: member.$.CWE_ID });
                }
                ;
            }
            else {
                cweMembers.push({ member: "No members available" });
            }
            ;
            if (items.Reference &&
                ...
                for (const reference of items.References[0].Reference) {
                    cwePublicReferences.push({ publicReference: reference.$.External_Reference_ID });
                }
                ;
            }
            else {
                cwePublicReferences.push({ publicReference: "No references available" });
            }
            ;
            cweOverall.push({
                cweID: ("CWE-" + items.$.ID),
