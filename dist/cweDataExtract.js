...
class cweFunctions {
    ...
    cweDataExtract(jsonData) {
        ...
        for (const items of jsonData.Weakness_Catalog.Weaknesses[0].Weakness) {
            ...
            if (items.Affected_Resources) {
                ...
                Affected_Resources.push({ Affected_Resources: "No Affected Resources" });
            }
            ;
            if (items.Functional_Areas) {
                ...
                Functional_Areas.push({ Functional_Areas: "No Functional Areas" });
            }
            ;
            cweDescription.push({
                cweID: ("CWE-" + items.$.ID),
                ...
                ExtendedDescription: typeof items.Extended_Description === "string"
    ...
    ;
    cweCategoryDataExtract(jsonData) {
        const cweData = [];
        for (const items of jsonData.Weakness_Catalog.Categories[0].Category) {
            let cweDescription = [];
            let Modifications = [];
            let Notes = [];
            if (items.Content_History && items.Content_History[0].Modification) {
